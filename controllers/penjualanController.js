...
const penjualanController = {
   ...
   pengirimanView: async (req, res) => {
        try {
            ...
            const { tgl_mulai, tgl_sampai } = req.query;
            ...
            // 2. Query Dinamis untuk Riwayat Selesai
            let sqlRiwayat = `
            ...
            // 3. Tambahkan kondisi jika filter tanggal diisi
            if (tgl_mulai) {
                sqlRiwayat += " AND DATE(p.tanggal_pesan) >= ?";
                paramsRiwayat.push(tgl_mulai);
            }
            if (tgl_sampai) {
                sqlRiwayat += " AND DATE(p.tanggal_pesan) <= ?";
                paramsRiwayat.push(tgl_sampai);
            }

            sqlRiwayat += " ORDER BY p.id DESC LIMIT 20";

            const [riwayatSelesai] = await db.query(sqlRiwayat, paramsRiwayat);
            ...
            res.render('penjualan/pengiriman', {
                ...
                query: req.query // Oper nilai filter kembali ke EJS
            });
        } catch (error) {
            console.error(error);
    ...
    cetakLaporanPengiriman: async (req, res) => {
        try {
            ...
            let query = `
                ...
                WHERE (p.status_pesanan = 'selesai' OR p.status_pesanan = 'terkirim')
            `;
            let params = [];
            ...
            const [riwayatSelesai] = await db.query(query, params);
    ...
    cetakExcelPengiriman: async (req, res) => {
        try {
            const db = require('../config/db');
            const ExcelJS = require('exceljs'); // Pastikan modul di-*require* jika belum di-global
            const { tgl_mulai, tgl_sampai } = req.query;
            ...
            riwayatSelesai.forEach((item, index) => {
                const tgl = new Date(item.waktu_pengiriman).toLocaleDateString('id-ID');
                worksheet.addRow({
                    no: index + 1,
                    id: `INV-${item.id}`,
                    tanggal: tgl,
